const cartReducer = (state, action) => {
  if (action.type === "ADD_TO_CART") {
    ...
    // Include weight in unique ID if available
    const selectedWeight = product.selectedWeight || '';
    const uniqueId = id + color + selectedWeight;

    // tackle the existing product

    let existingProduct = state.cart.find(
      (curItem) => curItem.id === uniqueId
    );
    ...
    if (existingProduct) {
      ...
      if (selectedWeight && product.variants) {
      ...
      let cartProduct = {
        id: uniqueId,
        productId: id, // Original product ID
        name: product.name,
        color,
        weight: selectedWeight, // Store selected weight
        amount,
        image: displayImage,
        price: product.price,
        max: maxStock,
